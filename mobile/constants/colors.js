// FOREST
const FOREST = {
  primary: "#4CAF50",
  textPrimary: "#2e5a2e",
  textSecondary: "#688f68",
  textDark: "#1b361b",
  placeholderText: "#767676",
  background: "#e8f5e9",
  cardBackground: "#f1f8f2",
  inputBackground: "#f4faf5",
  border: "#c8e6c9",
  starColor: "#FFD700",
  white: "#ffffff",
  black: "#000000",
};

// RETRO
const RETRO = {
  primary: "#e17055",
  textPrimary: "#784e2d",
  textSecondary: "#a58e7c",
  textDark: "#50372a",
  placeholderText: "#767676",
  background: "#ede1d1",
  cardBackground: "#faf5eb",
  inputBackground: "#f7f2ea",
  border: "#e2d6c1",
  starColor: "#FFD700",
  white: "#ffffff",
  black: "#000000",
};

// OCEAN
const OCEAN = {
  primary: "#1976D2", 
  textPrimary: "#1a4971",
  textSecondary: "#6d93b8", 
  textDark: "#0d2b43", 
  placeholderText: "#767676",
  background: "#e3f2fd", 
  cardBackground: "#f5f9ff",
  inputBackground: "#f0f8ff", 
  border: "#bbdefb",
  starColor: "#FFD700",
  white: "#ffffff",
  black: "#000000",
};

// BLOSSOM
const BLOSSOM = {
  primary: "#EC407A", 
  textPrimary: "#7d2150", 
  textSecondary: "#b06a8f", 
  textDark: "#5a1836", 
  placeholderText: "#767676",
  background: "#fce4ec", 
  cardBackground: "#fff5f8", 
  inputBackground: "#fef8fa", 
  border: "#f8bbd0",
  starColor: "#FFD700",
  white: "#ffffff",
  black: "#000000",
};

// Export all themes
export const THEMES = {
  forest: { name: "Forest", colors: FOREST },
  retro: { name: "Retro", colors: RETRO },
  ocean: { name: "Ocean", colors: OCEAN },
  blossom: { name: "Blossom", colors: BLOSSOM },
};

// Keep it simple - just export THEMES and the color objects
export default BLOSSOM;